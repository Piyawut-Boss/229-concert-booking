...
// ========== In-Memory Database ==========
// Temporary in-memory storage while PostgreSQL is being configured
let concerts = [
  {
    id: 1,
...
app.post('/api/admin/concerts', (req, res) => {
  ...
  const newConcert = {
    id: Math.max(...concerts.map(c => c.id), 0) + 1,
    name,
    artist,
    date,
    venue,
...
app.listen(PORT, async () => {
  ...
  console.log(`💾 Database: In-Memory (Data persists for this session only)`);
