...
function HomePage() {
  ...
  const [sliderParams, setSliderParams] = useState({ cardWidth: 0, gap: 20, setWidth: 0 })
  ...
  const infiniteConcerts = [...originalRecommended, ...originalRecommended, ...originalRecommended, ...originalRecommended]
  ...
  const updateSliderMetrics = () => {
    if (sliderRef.current && sliderRef.current.children.length > 0) {
      ...
      if (sliderRef.current.scrollLeft === 0) sliderRef.current.scrollLeft = setW;
    }
  }
  ...
  const handleScroll = () => {
    ...
    if (scrollLeft >= setWidth * 2) sliderRef.current.scrollLeft = scrollLeft - setWidth;
    else if (scrollLeft <= 0) sliderRef.current.scrollLeft = setWidth;
  }

  const slideLeft = () => { if (sliderRef.current) sliderRef.current.scrollBy({ left: -(sliderParams.cardWidth + sliderParams.gap), behavior: 'smooth' }) }
  const slideRight = () => { if (sliderRef.current) sliderRef.current.scrollBy({ left: (sliderParams.cardWidth + sliderParams.gap), behavior: 'smooth' }) }
  ...
  const handleBookingSuccess = () => { setShowModal(false); fetchConcerts(); }
    ...
    return (
      <div key={`${concert.id}-${index}`} className={`ticket-card ${isSoldOut || isClosed ? 'disabled' : ''}`}>
        <div className="ticket-left">
          <img src={concert.imageUrl} alt={concert.name} className="concert-poster" onError={(e) => {e.target.src = 'https://via.placeholder.com/300x450?text=No+Image'}} />
        </div>
        <div className="ticket-right">
          <h2 className="concert-name">{concert.name}</h2>
          <p className="artist-name">{concert.artist}</p>
          <div className="meta-info">
            <div className="meta-item"><span>📅 {date} • {time}</span></div>
            <div className="meta-item"><span>📍 {concert.venue}</span></div>
          </div>
          <div className="ticket-action">
             {/* ... (รายละเอียดเดิม) ... */}
             <div className="price-tag"><span className="amount">฿{concert.price.toLocaleString()}</span></div>
             <button className={`book-btn ${isClosed || isSoldOut ? 'disabled' : ''}`} onClick={() => handleBooking(concert)}>{isClosed ? 'Closed' : isSoldOut ? 'Sold Out' : 'Buy Ticket'}</button>
          </div>
        </div>
      </div>
    )
  ...
  const renderHorizontalConcertCard = (concert) => {
    ...
    return (
      <div key={concert.id} className={`horizontal-card ${isSoldOut || isClosed ? 'disabled' : ''}`}>
        {/* รูปด้านซ้าย */}
        ...
        <div className="horizontal-right">
          <div className="horizontal-info">
            <h3>{concert.name}</h3>
            <div className="horizontal-artist">{concert.artist}</div>
            ...
            <div className="hz-meta-item location">
          ...
          <div className="horizontal-action">
            {/* ถ้าอยากได้ปุ่มแดงเหมือนรูป Ref ให้แก้ class ปุ่มใน CSS */}
            {!isClosed && !isSoldOut ? (
               <button className="hz-book-btn" onClick={() => handleBooking(concert)}>ซื้อบัตร</button>
            ) : (
               <button className="hz-book-btn disabled" disabled>{isClosed ? 'ปิดรับ' : 'บัตรหมด'}</button>
            )}
          </div>
        </div>
      </div>
